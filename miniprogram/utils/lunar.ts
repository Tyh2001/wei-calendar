export const lunarCalendar = {
   ...
   * @return Number
   */
  solarMonth: [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] as const,
  ...
  //  * 返回默认定义的阳历节日
  //  */
  // getFestival: function (): festivalType {
   ...
   * 返回默认定义的内容里节日
   */
  // getLunarFestival: function (): lFestivalType {
  ...
  //  */
  // setFestival: function (param = { }): void {
  //   this.festival = param
  // },
   ...
   */
  solarDays: function (y: number, m: number): 31 | 28 | 30 | 29 | -1 {
    if (m > 12 || m < 1) {
      return -1
    } // 若参数错误 返回-1
    const ms = m - 1
   ...
   * @return Cn string
   */
  toGanZhiYear: function (lYear: number): string {
   ...
   * @return Cn string
   */
  toConstellation: function (cMonth: number, cDay: number): string {
    const s =
   ...
   */
  toChinaMonth: function (m: number): string | number {
    // 月 => \u6708
    if (m > 12 || m < 1) {
      return -1
    } // 若参数错误 返回-1
   ...
   */
  solar2lunar: function (yPara: number, mPara: number, dPara: number): object | number {
    ...
    const cWeek: string = this.nStr1[nWeek]
    // 数字表示周几顺应天朝周一开始的惯例
    if (nWeek === 0) {
      nWeek = 7
    }
    ...
    // 该日期所属的星座
    const constellation = this.toConstellation(m, d)
   ...
   */
  lunar2solar: function (y: number, m: number, d: number, isLeapMonth: boolean): object | number {
    ...
    const strap: number = Date.UTC(1900, 1, 30, 0, 0, 0)
    const calObj: Date = new Date((offset + d - 31) * 86400000 + strap)
    const cY: number = calObj.getUTCFullYear()
    const cM: number = calObj.getUTCMonth() + 1
    const cD: number = calObj.getUTCDate()
